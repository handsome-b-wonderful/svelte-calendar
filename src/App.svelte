...
<div class="container">
  ...
  <p>
    Above you can see the default styling of this component. This will be
    created for you by default when using the component but you can also pass in
    your own calendar 'trigger' either as a slot (custom element or svelte) or
    as a DOM node reference (use as vanilla JS). Here are some examples:
  </p>
...
showClear="true"
  ...
  <div class="text-center">
    <Datepicker
      format={dateFormat}
      bind:formattedSelected={datePicked}
      bind:dateChosen={isDatePicked}
      showToday="true"
      showClear="true"
    >
      <button class="today-clear-button">
        {#if isDatePicked} {datePicked} {:else} Select a Date {/if}
      </button>
    </Datepicker>
  </div>
  ...
  <p class="note">
    Note: The calendar will only generate dates up until the end date, so it is
    recommended to set this value to whatever is useful for you.
  </p>

  <p>
    You can also provide a `selectableCallback` prop which can be used to mark
    individual days between `start` and `end` as selectable. This callback
    should accept a single date as an argument and return true (if selectable)
